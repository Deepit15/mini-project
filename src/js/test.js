...
//Get an employees
app.get("/customeremail/:email", (req, res) => {
  mysqlConnection.query(
    "SELECT * FROM customer WHERE cus_email = ? ORDER BY cus_id DESC",
    [req.params.email],
    (err, rows, fields) => {
      if (!err) res.send(rows);
      else console.log(err);
...
app.get("/food_order_cus/:cus", (req, res) => {
  mysqlConnection.query("SELECT * FROM food_order where cus_id = ? order by order_id DESC", [req.params.cus], (err, rows, fields) => {
    if (!err) res.send(rows);
    else console.log(err);
  });
});
...
app.post("/food_orderinsert", (req, res) => {
  let order = req.body.food_order;
  console.log(order.cus_id);
  mysqlConnection.query(
    "insert into food_order(price, waiter_id,  cus_id ,table_id) values(" +
      order.price +
      "," +
      order.waiter_id +
      "," +
...
app.post("/tableupdateotp", (req, res) => {
  let table = req.body.table;
  console.log(table);
  mysqlConnection.query(
    "update table_mng set  OTP = " +
      table.otp +
      " where table_id = " +
      table.table_id +
      ";",
...
app.get("/food_sub_orders/:id", (req, res) => {
  mysqlConnection.query(
    "SELECT * FROM food_sub_order WHERE order_id = ?",
    [req.params.id],
    (err, rows, fields) => {
      if (!err) res.send(rows);
      else console.log(err);
