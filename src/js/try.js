App = {
  ...
  getpatcustomeremail: function (i) {
    var x = document.getElementById("changeRecordForm");
    const api_url = "http://localhost:3000/customeremail/" + i;
    // console.log(api_url);
    data = this.getapi(api_url).then((data) => {
      console.log("in");
      sessionStorage.setItem("cus_id", data[0].cus_id);
      console.log(data[0].cus_id);
      // alert(data[0].cus_id);
      // $("#changedata").modal();
    });
  },
  ...
  postnewcustomer:async function () {
    var xhr = new XMLHttpRequest();
    alert("in");
    var dta = JSON.stringify({
      customer: {
        cus_name: document.getElementById("username").value,
        cus_no: document.getElementById("phone-number").value,
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4) {
        // alert("called");
        // App.getpatcustomeremail(document.getElementById("email").value);
        // alert("returned");
      }
    };
    // $("#changedata").trigger("reset");
  ...
  postnewfoodorder: function (waiter_id,price,status,order_time,cus_id,table_id) {
    var xhr = new XMLHttpRequest();
    var dta = JSON.stringify({
      food_order: {
        waiter_id: waiter_id,
  ...
  getpatotptable: async function (i) {
    var x = document.getElementById("changeRecordForm");
    const api_url = "http://localhost:3000/table/" + i;
    // console.log(api_url);
    data = this.getapi(api_url).then((data) => {
      localStorage.getItem("otp","OTP");
        // console.log(data[0].OTP)
        sessionStorage.setItem("otp",data[0].OTP);
        if(data[0].status == "vacant")
        {
          App.postnewfoodorder(data[0].waiter_id,0,"reserve",Date().toLocaleString(),sessionStorage.getItem("cus_id"),i);
        }
        // return  ;
    });
  },
  ...
  postuptableotp: function (id,otp) {
    var xhr = new XMLHttpRequest();
    var dta = JSON.stringify({
      table: {
        table_id: id,
    ...
    xhr.open("POST", "http://localhost:3000/tableupdateotp", true);
    xhr.setRequestHeader("Content-Type", "application/json");
    console.log(dta);
    xhr.send(dta);
  },
  ...
  getpatfood_order: function (i) {
    var x = document.getElementById("changeRecordForm");
    const api_url = "http://localhost:3000/food_order_cus/" + i;
    // console.log(api_url);
    data = this.getapi(api_url).then((data) => {
      sessionStorage.setItem("order_id", data[0].order_id);
  ...
  getpatfood_sub_orders_cart: function (arr) {
    // alert(arr);
    sessionStorage.setItem("order",arr);
    window.location = 'cart.html';
    
    // console.log(arr);
    arr = sessionStorage.getItem("order");
    // alert(arr);
    // console.log(arr.object[1]);
    // var x = document.getElementById("changeRecordForm");
    var id = sessionStorage.getItem("order_id");
    console.log(id);
    const api_url = "http://localhost:3000/food_sub_orders/" + id;
    alert("api call");
    // console.log(api_url);
    data = this.getapi(api_url).then((data) => {
      alert("in api call");
    ...
    {
      alert("in for");
      console.log(arr[i].menu_id+" "+arr[i].qty);
      var a = "<tr ><td>"+
      arr[i].menu_id +
      "</td><td >" +
      arr[i].qty +"</td></tr>";
      document.getElementById("order_item").innerHTML += a;
    }
    // console.log(data);
  },
